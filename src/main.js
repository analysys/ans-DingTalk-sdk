...
class Ark_PASS_SDK extends API {
    ...
    set requestDataType (dataType) {
        ...
        if (Util.paramType(dataType) !== 'String') {
            baseConfig.status.FnName = "requestDataType";
            baseConfig.status.errorCode = "60001";
            baseConfig.status.key = dataType;
            errorLog();
            return;
        }
        let dataTypeList = ["json", 'text', 'base64', 'arraybuffer'] //arraybuffer   10.1.70 版本开始支持）
        let dataTypeStr = dataType.toLowerCase()
        if (dataTypeList.indexOf(dataTypeStr) > -1) {
            baseConfig.status.FnName = "requestDataType";
            baseConfig.status.successCode = "20002";
            baseConfig.status.value = dataTypeStr;
            successLog();
            baseConfig.base.dataType = dataTypeStr;
        } else {
            baseConfig.status.FnName = "requestDataType";
            baseConfig.status.errorCode = "600022";
            baseConfig.status.key = dataType;
            errorLog();
        }
    }
    get requestDataType () {
        return baseConfig.base.dataType;
    }
}
